...
def classify_images(images_dir, results_dic, model):
    ...
    """
     # Loop over each image in the results dictionary
...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
        ...
        # Convert the classifier label to lowercase and strip whitespace
        classifier_label = classifier_label.lower().strip()

        # Compare the pet label with the classifier label using the 'in' operation
        match = 1 if pet_label in classifier_label else 0

        # Extend the results_dic with the classifier label and match result
