...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    # Create dictionary for fast lookup of dog names
    dognames_dic = dict()

    # Read dog names from dogfile and populate dognames_dic
    with open(dogfile, "r") as infile:
        for line in infile:
            # Clean up the line by stripping newlines and extra spaces
            dog_name = line.strip().lower()
            if dog_name not in dognames_dic:
                dognames_dic[dog_name] = 1

    # Loop through the results_dic and adjust for "is-a-dog" or "is-NOT-a-dog"
